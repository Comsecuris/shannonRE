...
def def_functions(s_start):
    ...
    while (s_addr < s_end):
        ...
        if (idaapi.get_func(s_addr)):
            ...
            '''
            #also add STMFD 
            if ((idc.print_insn_mnem(s_addr) == "STM") and ("SP!" in idc.print_operand(s_addr, 0)) and ("LR" in idc.print_operand(s_addr, 1))) or (((idc.print_insn_mnem(s_addr) == "PUSH") or (idc.print_insn_mnem(s_addr) == "PUSH.W") or (idc.print_insn_mnem(s_addr) == "STR.W") ) and ("LR" in idc.print_operand(s_addr, 0))):
